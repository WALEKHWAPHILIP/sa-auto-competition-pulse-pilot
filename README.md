...
A **research-engineering pilot** that turns South African automotive news into a **monthly competition-event time series** and links it to **monthly new-vehicle sales (NAAMSA)**. The repo is designed so an interviewer can run the pipeline, inspect raw snapshots, verify transformations, and reproduce the final tables/figures.
...
## Next steps (tight, employer-facing)
